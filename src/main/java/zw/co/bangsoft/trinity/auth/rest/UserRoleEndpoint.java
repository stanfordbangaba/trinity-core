...
public class UserRoleEndpoint {
	...
	public Response findById(@PathParam("id") Long id) {
		TypedQuery<UserRole> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT u FROM UserRole u LEFT JOIN FETCH u.user LEFT JOIN FETCH u.role WHERE u.id = :entityId ORDER BY u.id",
						UserRole.class);
	...
	public List<UserRole> listAll(@QueryParam("start") Integer startPosition,
		...
		TypedQuery<UserRole> findAllQuery = em
				.createQuery(
						"SELECT DISTINCT u FROM UserRole u LEFT JOIN FETCH u.user LEFT JOIN FETCH u.role ORDER BY u.id",
						UserRole.class);
